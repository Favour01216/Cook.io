...
// Select the search field element
const $searchField = document.querySelector("[data-search-field]");
const $searchBtn = document.querySelector("[data-search-btn]");

// Check if the search field exists before adding the event listener
...
// Initialize with the first tab button
...
const addTabContent = ($currentTabBtn, $currentTabPanel) => {
  ...
  fetchData(
    ...
    function (data) {
      ...
      for (let i = 0; i < 12; i++) {
        const {
          recipe: { uri, image, label: title, totalTime: cookingTime },
        } = data.hits[i];
