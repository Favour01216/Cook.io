...
const initAccordion = function ($element) {
  const /**{NodeElements} */ $button = $element.querySelector(
      "[data-accordion-btn]"
    );
...
$filterSubmit.addEventListener("click", function () {
  ...
  window.location.href = "/recipes.html" + queryString;
});
      ...
      <div class="card-body">
          ...
          <div class="meta-wrapper">
              ...
              <span class="label-medium">${
                cookTime ? getTime(cookTime).time || "<1" : ""
...
fetchData(queries || defaultQueries, (data) => {
  ...
  if (data.hits.length) {
    ...
    $gridList.innerHTML = `<p class="body-medium info-text">No recipes found </p>`;
  }
});
...
window.addEventListener("scroll", async (e) => {
  ...
  if (!nextPageUrl)
    $loadMore.innerHTML = `<p class="body-medium info-text">No more recipes found </p>`;
});
